...
shell-cannot-run-command-from-server = Você não pode executar este comando a partir do servidor.
...
shell-need-exactly-one-argument = Necessita exatamente um argumento.
shell-wrong-arguments-number-need-specific = Necessita {$properAmount} argumentos, havia {$currentAmount}.
...
shell-need-between-arguments = Necessita de {$lower} a {$upper} argumentos!
shell-need-minimum-arguments = Necessita de pelo menos {$minimum} argumentos!
shell-need-minimum-one-argument = Necessita de pelo menos um argumento!
...
shell-timespan-minutes-must-be-correct = {$span} não é um timespan de minutos válido.
