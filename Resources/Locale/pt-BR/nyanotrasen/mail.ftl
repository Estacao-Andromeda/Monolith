...
mail-desc-far = Um pacote de correio.
mail-desc-close = Um pacote de correio endereçado a {CAPITALIZE($name)}, {$job}. Última localização conhecida: {$station}.
mail-desc-fragile = Ele tem um [color=red]etiqueta frágil vermelha[/color].
...
mail-item-name-unaddressed = correio
mail-item-name-addressed = correio ({$recipient})

# Frontier: descrição reformulada, não precisa ser um contêiner.
command-mailto-description = Coloca um item na fila para ser entregue a um destinatário. Exemplo de uso: `mailto 1234 5678 false false`. Se a entidade alvo for um contêiner, seu conteúdo será transferido para um pacote de correio real.
# Frontier: adicionar descrição is-large, contêiner<conteúdo
...
command-mailto-invalid-container = A entidade do contêiner alvo não tem um contêiner {$requiredContainer}.
command-mailto-unable-to-receive = A entidade do destinatário alvo não pôde ser configurada para receber correio. A ID pode estar ausente.
command-mailto-no-teleporter-found = A entidade do destinatário alvo não pôde ser correspondida a nenhum teletransportador de correio da estação. O destinatário pode estar fora da estação.
...
command-mailnow = Força todos os teletransportadores de correio a entregarem outra rodada de correio o mais rápido possível. Isso não contornará o limite de correio não entregue.
command-mailnow-help = Uso: {$command}
command-mailnow-success = Sucesso! Todos os teletransportadores de correio estarão entregando outra rodada de correio em breve.
