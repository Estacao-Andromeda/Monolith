# ====================================================================
# Entity Localization File
# Generated: 2026-07-10 21:56:05
# Total entities: 3
# ====================================================================

# From: _CorvaxNext/Actions/station_ai.yml

ent-ActionBackToAi = Voltar para IA
    .desc = Envia sua mente de volta ao núcleo.

ent-ActionOpenRemoteDevicesMenu = Abrir lista de dispositivos remotos
    .desc = Abre um menu com dispositivos remotos disponíveis.

# From: _CorvaxNext/Entities/Objects/Specific/Robotics/airemote.yml

ent-AiRemoteBrain = Dispositivo de Rede IA-Ciborgue
    .desc = Placa de controle de ciborgue de Inteligência Artificial.

